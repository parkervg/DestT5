...
class Spider(datasets.GeneratorBasedBuilder):
    ...
    def __init__(
        self, *args, writer_batch_size=None, spider_dataset_url=_URL, **kwargs
    ) -> None:
        super().__init__(*args, writer_batch_size=writer_batch_size, **kwargs)

        self.schema_cache = dict()
        self.include_train_others: bool = kwargs.pop("include_train_others", False)
        self._url = spider_dataset_url
    ...
    def _split_generators(
        ...
        downloaded_filepath = dl_manager.download_and_extract(url_or_urls=self._url)
        logger.info("-" * 10)
        logger.info("Downlaoded path:")
        logger.info(downloaded_filepath)
        logger.info(os.path.join(downloaded_filepath, "spider/train_spider.json"))
        logger.info("-" * 10)
