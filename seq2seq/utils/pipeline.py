...
class Text2SQLGenerationPipeline(Text2TextGenerationPipeline):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.schema_serialization_type: str = kwargs.pop(
            "schema_serialization_type", "peteshaw"
        )
        self.schema_serialization_randomized: bool = kwargs.pop(
            "schema_serialization_randomized", False
        )
        self.schema_serialization_with_db_id: bool = kwargs.pop(
            "schema_serialization_with_db_id", True
        )
        self.schema_serialization_with_db_content: bool = kwargs.pop(
    ...
    def __call__(
        self, inputs: Union[Text2SQLInput, List[Text2SQLInput]], *args, **kwargs
    ):
        r"""
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        encodings = self._parse_and_tokenize(
            inputs, *args, truncation=truncation, **kwargs
        )
        print()
        print(Fore.CYAN + "Serialized input:")
        print(Fore.CYAN + self.tokenizer.decode(encodings.input_ids[0, :]))
        print()
        return encodings
    ...
    def _pre_process(self, input: Text2SQLInput) -> str:
        ...
        if input.db_id not in self.schema_cache:
            self.schema_cache[input.db_id] = get_schema(
                db_path=self.db_path, db_id=input.db_id
            )
        ...
        return spider_get_input(
            question=input.utterance, serialized_schema=serialized_schema, prefix=prefix
        )
