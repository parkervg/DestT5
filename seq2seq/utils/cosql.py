...
from .trainer import EvalPrediction, Seq2SeqTrainer
...
def cosql_pre_process_function(
    ...
    inputs = [
        cosql_get_input(
            utterances=utterances, serialized_schema=serialized_schema, prefix=prefix
        )
        for utterances, serialized_schema in zip(
            batch["utterances"], batch["serialized_schema"]
        )
    ]
