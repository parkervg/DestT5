...
def spider_concept_get_target(
    gold_serialized_schema: str,
) -> str:
    return gold_serialized_schema[2:].strip()  # Index at 2: to cut off initial '|'


def spider_add_serialized_schema(
    ex: dict, data_training_args: DataTrainingArguments
) -> dict:
    if (
        data_training_args.use_gold_concepts
        and data_training_args.task_type == "schema_prediction"
    ):
        raise ValueError("Both use_gold_concepts and concept_prediction can't be true")
    gold_serialized_schema = None
    if (
        data_training_args.use_serialization_file is not None
        and not data_training_args.splash_train_with_spider
    ):
...
def spider_pre_process_function(
    ...
    if data_training_args.task_type == "schema_prediction":
        logger.warning("Encoding concept_prediction model")
    ...
    if data_training_args.task_type == "schema_prediction":
        ...
        """
        targets = [
            spider_concept_get_target(gold_serialized_schema=gold_serialized_schema)
