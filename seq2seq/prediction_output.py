...
from .utils.dataset import DataTrainingArguments
from .utils.pipeline import (
    ConversationalText2SQLGenerationPipeline,
    ConversationalText2SQLInput,
    Text2SQLGenerationPipeline,
    Text2SQLInput,
)
...
class PredictionOutputArguments:
    ...
    inputs_path: str = field(
        default="data/dev.json", metadata={"help": "Where to find the inputs"}
    )
    output_path: str = field(
        default="predicted_sql.txt",
        metadata={"help": "Where to write the output queries"},
    )
    device: int = field(
        default=0,
        metadata={
    ...
    conversational: bool = field(
        default=False, metadata={"help": "Whether or not the inputs are conversations"}
    )
...
def main(combined_args):
    ...
    parser = HfArgumentParser(
        (PicardArguments, PredictionOutputArguments, DataTrainingArguments)
    )
    picard_args: PicardArguments
    prediction_output_args: PredictionOutputArguments
    data_training_args: DataTrainingArguments
    picard_args, prediction_output_args, data_training_args = parser.parse_json_file(
        json_file=combined_args
    )

    if os.path.isfile(prediction_output_args.output_path):
        raise RuntimeError(
            "file `{}` already exists".format(prediction_output_args.output_path)
        )
...
def text2sql(model, tokenizer, prediction_output_args, data_training_args):
    ...
    with alive_bar(len(questions)) as bar:
        for question in questions:
            try:
                outputs = pipe(
                    inputs=Text2SQLInput(question["question"], question["db_id"])
                )
                output = outputs[0]
                ...
                query = ""
            logger.info(
                "writing `{}` to `{}`".format(query, prediction_output_args.output_path)
            )
            bar.text(query)
            bar()
        ...
        for conversation in conversations:
            ...
            for turn in conversation["interaction"]:
                utterances.extend(
                    (
                        utterance.strip()
                        for utterance in turn["utterance"].split(sep="|")
                    )
                )
                try:
                    outputs = pipe(
                        inputs=ConversationalText2SQLInput(
                            list(utterances), db_id=conversation["database_id"]
                        )
                    )
                    output = outputs[0]
                    query = output["generated_text"]
                ...
                logger.info(
                    "writing `{}` to `{}`".format(
                        query, prediction_output_args.output_path
                    )
                )
                bar.text(query)
                bar()
